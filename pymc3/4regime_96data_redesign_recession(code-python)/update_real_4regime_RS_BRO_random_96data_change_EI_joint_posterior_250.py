...
def SwitchingBRO_streaming(seed):
    ...
    df0 = pd.read_csv('/root/rs_bro92/RSDR_CVaR_dataset/weathers_state.csv')
    ...
    #df_used = df0[(df0['Date'] >= 20000101) & (df0['Date'] < 20080301)].reset_index(drop=True)
    ...
    for t in range(n_timestep):
        ...
        for ego_iter in range(n_iteration):
            ...
            # 保存为 CSV 文件
            log_df.to_csv('log_inventory_4reg_RS_BRO_change_EI_pos_250.csv', mode='a', header=not os.path.exists('log_inventory_4reg_RS_BRO_change_EI_pos_250.csv'), index=False)
...
cmd = ['-t','60000201-RS-BRO-random-change-EIjoint-pos-250-4regime-real-96data-seed-44-49', 
       ...
       '-n_i', '30',###30  
       '-n_rep', '10',###10  
       '-n_candidate','250',###250 (250*4=1000)
       '-window', 'all',
       '-n_MCMC', '100',##100
       '-n_components', '4', ###4
