...
def SwitchingBRO_streaming(seed):
    ...
    initial_posterior_0 = bayesian_hmm_posterior_general(n_initial, seed, xi[:n_xi], n_components, alpha=lam_prior_alpha, beta=lam_prior_beta) 
    ...
    MCMC_posterior_0 = bayesian_hmm_posterior_general(sample_count, seed_mcmc, xi[:n_xi], n_components, alpha=lam_prior_alpha, beta=lam_prior_beta) 
    ...
    for t in range(n_timestep):
        ...
        for ego_iter in range(n_iteration):
            ...
            df.to_csv('log_real_inventory.csv', mode='a', header=not os.path.exists('log_real_inventory.csv'), index=False)
        ...
        MCMC_posterior_0 = bayesian_hmm_posterior_general(sample_count, seed_mcmc+t+1, xi[:n_xi+t+1], n_components, alpha=lam_prior_alpha, beta=lam_prior_beta)
...
cmd = ['-t','20250509-RS-BRO-random-2regime-real-48data-seed-60-69', 
