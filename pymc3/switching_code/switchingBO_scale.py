...
def generate_EGO_log_df_RS_BRO(seed, t, ego_iter, lambda_new, lambdas_all, weights_all):
    """
    构建列顺序为：meta info + all lambdas + all weights
    """
    N_MC, n_components = lambdas_all.shape
    records = []
...
# scale here, to improve the performance of gp model
class CustomMinMaxScalerMultiDim:
    def __init__(self, min_vals, max_vals):
        self.min_vals = np.asarray(min_vals)
        self.max_vals = np.asarray(max_vals)
        self.range = self.max_vals - self.min_vals
        if np.any(self.range == 0):
            raise ValueError("Each dimension must have non-zero range.")
    ...
    def inverse_transform(self, X_scaled):
        ...
        return X_scaled * self.range + self.min_vals

def predicted_mc_mean_vectorized_scaled(X_, lambdas_all, weights_all, gp, scaler):
    """
    Compute Monte Carlo averaged posterior mean of GP at input X_, using scaled inputs.

    Parameters:
    - X_: (N_samples, d) array of input design points (original scale)
    - lambdas_all: (N_MC, n_components) array of posterior lambda samples (original scale)
    - weights_all: (N_MC, n_components) array of posterior weights
    - gp: trained GaussianProcessRegressor on scaled (x, lambda)
    - scaler: fitted CustomMinMaxScalerMultiDim object
    ...
    # Step 1: Flatten lambda samples (not taking inverse anymore!)
    lambdas_flat = lambdas_all.reshape(-1, 1)  # shape: (N_MC * n_components, 1)

    # Step 2: Tile for each x
    lambdas_tiled = np.tile(lambdas_flat, (N_samples, 1))  # (N_samples * N_MC * n_components, 1)

    # Step 3: Repeat X_
    X_repeat = np.repeat(X_, N_MC * n_components, axis=0)  # (N_samples * N_MC * n_components, d)

    # Step 4: Concatenate (x, lambda)
    X_lambda_all = np.hstack((X_repeat, lambdas_tiled))  # shape: (N_samples * N_MC * n_components, d+1)

    # Step 5: Scale inputs
    X_lambda_scaled = scaler.transform(X_lambda_all)

    # Step 6: GP prediction
    m_all = gp.predict(X_lambda_scaled).reshape(N_samples, N_MC, n_components).transpose(1, 2, 0)  # (N_MC, n, N_samples)

    # Step 7: Weighted average over MC samples
    weighted_means = np.sum(weights_all[:, :, None] * m_all, axis=1)  # (N_MC, N_samples)
    mu = np.mean(weighted_means, axis=0)  # (N_samples,)

    return mu

def predicted_mc_mean_and_approx_var_vectorized_scaled(X_, lambda_, lambdas_all, weights_all, gp, n_rep, scaler):
    """
    Compute MC posterior mean and approximate variance under multiple regimes,
    using scaled inputs without 1/lambda.

    Parameters:
    - X_: (N_samples, d) input points
    - lambda_: (N_samples,) point estimate for each x (original scale)
    - lambdas_all: (N_MC, n_components) posterior samples (original scale)
    - weights_all: (N_MC, n_components)
    - gp: trained GP on scaled (x, lambda)
    - n_rep: number of simulation replications
    - scaler: fitted CustomMinMaxScalerMultiDim

    Returns:
    - mu: (N_samples,)
    - tilde_sigma: (N_samples,)
    """
    N_samples, d = X_.shape
    N_MC, n_components = lambdas_all.shape

    ### --- Construct input (x, lambda) combinations ---  🔧 改动：去掉倒数
    lambdas_flat = lambdas_all.reshape(-1, 1)  # (N_MC * n_components, 1)
    lambdas_tiled = np.tile(lambdas_flat, (N_samples, 1))  # (N_samples * N_MC * n_components, 1)
    X_repeat = np.repeat(X_, N_MC * n_components, axis=0)  # (N_samples * N_MC * n_components, d)
    X_lambda_all = np.hstack((X_repeat, lambdas_tiled))  # (N_samples * N_MC * n_components, d+1)

    ### 🔧 添加归一化
    X_lambda_scaled = scaler.transform(X_lambda_all)

    ### --- GP prediction
    m_all = gp.predict(X_lambda_scaled).reshape(N_samples, N_MC, n_components).transpose(1, 2, 0)
    weighted_means = np.sum(weights_all[:, :, None] * m_all, axis=1)
    mu = np.mean(weighted_means, axis=0)
    ...
    for idx in range(N_samples):
        x = X_[idx:idx+1, :]  # (1, d)
        lambda_j = lambda_[idx:idx+1]  # (1,)

        # 🔧 1. 不取倒数，直接拼接
        x_lambda_j = np.hstack([x, lambda_j.reshape(1, 1)])  # (1, d+1)

        # 🔧 2. 分子输入构造（仍旧是所有 MCMC 样本 lambda）
        lambdas_all_vec = lambdas_all.reshape(-1, 1)  # (N_MC * n, 1)
        x_rep = np.repeat(x, N_MC * n_components, axis=0)  # (N_MC * n, d)
        x_lambdas_all = np.hstack([x_rep, lambdas_all_vec])  # (N_MC * n, d+1)

        # 🔧 3. 归一化输入
        x_lambda_j_scaled = scaler.transform(x_lambda_j)  # (1, d+1)
        x_lambdas_all_scaled = scaler.transform(x_lambdas_all)  # (N_MC * n, d+1)

        # 4. 分母
        k_diag = gp.kernel_(x_lambda_j_scaled, x_lambda_j_scaled)[0, 0]
        denom = np.sqrt(k_diag + sigma_epsilon_sq / n_rep)

        # 5. 分子
        x_lambda_j_rep_scaled = np.repeat(x_lambda_j_scaled, N_MC * n_components, axis=0)
        k_vals = compute_diag_kernel(gp.kernel_, x_lambdas_all_scaled, x_lambda_j_rep_scaled)  # (N_MC * n,)

        k_vals = k_vals.reshape(N_MC, n_components)
        weighted_k = np.sum(weights_all * k_vals / denom, axis=1)  # (N_MC,)
        tilde_sigma[idx] = np.mean(weighted_k)

    return mu, tilde_sigma

def update_data_switching_EGO_joint_no_reciprocal(D_lambda_new, D_old, Y_old, n_rep, method, f, shape_x):

    D_new = D_lambda_new[:shape_x]
    lambda_new = D_lambda_new[shape_x:]
    distribution_new = {'dist': "exp", "rate": lambda_new}
...
def generate_EGO_log_df_RS_BRO_no_reciprocal(seed, t, ego_iter, lambda_new, lambdas_all, weights_all):
    """
    构建列顺序为：meta info + all lambdas + all weights
    """
    N_MC, n_components = lambdas_all.shape
