...
class synthetic_uni_problem: # updated on June 9, 2025
    
    def __init__(self, true_dist):
        self.dimension_x = 1
        self.dimension_p = 1
        self.lb = [0]
        self.ub = [50] ##change on Jun9
        self.true_dist = true_dist  
        self.x_star = None
        self.f_star = None
    ...
    def evaluate_true(self, xx):
        ...
        """
        x = xx[0]
        value = synthetic_true_objective1(x, self.true_dist.get("rate"))
        return value
...
def synthetic_simulator1(x, P, n_rep, method, **kwargs):
    ...
    if method == 'bayesian_hist':  # KDE-based sampling from historical data
        xi_samples = kde_sample(P, size=n_rep, **kwargs)
...
def synthetic_objective1(x, xi):
    ##(updated on June 9, 2025) follow SIAM Enlu Zhou, a little change
    return (x - xi)**2 + 10*xi

def synthetic_true_objective1(x, lam):

    return (x - 1/lam)**2 + 10/lam + (1/lam)**2 
