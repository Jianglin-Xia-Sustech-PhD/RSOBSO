...
def predicted_mc_mean_vectorized_no_inverse(X_, lambdas_all, weights_all, gp):
    """
    Compute Monte Carlo averaged posterior mean of GP at input X_, 
    without taking the inverse of lambda values.
    ...
    # Step 1: Flatten lambda samples (no inverse)
    lambdas_flat = lambdas_all.reshape(-1)  # shape: (N_MC * n_components,)

    # Step 2: Tile for each x
    lambdas_tiled = np.tile(lambdas_flat, N_samples).reshape(-1, 1)  # (N_samples * N_MC * n, 1)

    # Step 3: Repeat X_
    X_repeat = np.repeat(X_, N_MC * n_components, axis=0)  # (N_samples * N_MC * n, d)

    # Step 4: Final GP input
    X_lambda_all = np.hstack((X_repeat, lambdas_tiled))  # (N_samples * N_MC * n, d+1)
...
def predicted_mc_mean_and_approx_var_vectorized_no_inverse(X_, lambda_, lambdas_all, weights_all, gp, n_rep):
    N_samples, d = X_.shape
    N_MC, n_components = lambdas_all.shape

    ### --- Construct input (x, lambda) combinations ---
    # Step 1: Prepare all lambda as flat vector: shape (N_MC * n_components,)
    lambdas_flat = lambdas_all.reshape(-1)

    # Step 2: Tile this to match X_: final shape (N_samples * N_MC * n_components, 1)
    lambdas_tiled = np.tile(lambdas_flat, N_samples).reshape(-1, 1)
    ...
    X_lambda_all = np.hstack((X_repeat, lambdas_tiled))
    ...
    for idx in range(N_samples):
        ...
        lambda_j = lambda_[idx:idx+1]  # shape: (1,)

        # 1. 构造分母输入
        x_lambda_j = np.hstack([x, lambda_j.reshape(-1, 1)])  # shape: (1, d+1)

        # 2. 构造分子输入
        lambdas_all_flat = lambdas_all.reshape(-1, 1)  # shape: (N_MC * n_components, 1)
        x_rep = np.tile(x, (N_MC * n_components, 1))   # shape: (N_MC * n_components, d)
        x_lambdas_all = np.hstack([x_rep, lambdas_all_flat])  # shape: (N_MC * n_components, d+1)

        # 3. 计算分母 k((x_j, lambda_j), (x_j, lambda_j)) + noise
        k_diag = gp.kernel_(x_lambda_j, x_lambda_j)  # shape: (1,1)
        sigma_epsilon_sq = gp.kernel_.k2.noise_level  # noise from kernel
        denom = np.sqrt(k_diag[0, 0] + sigma_epsilon_sq / n_rep)

        # 4. 计算分子 k((x_j, lambda_l), (x_j, lambda_j))
        x_lambda_j_expanded = np.repeat(x_lambda_j, N_MC * n_components, axis=0)
        k_vals = compute_diag_kernel(gp.kernel_, x_lambdas_all, x_lambda_j_expanded)  # shape: (N_MC * n_components,)

        # 5. 加权求和
        k_vals = k_vals.reshape(N_MC, n_components)
        weighted_k = np.sum(weights_all * k_vals / denom, axis=1)  # shape: (N_MC,)
...
def generate_EGO_log_df_RS_BRO_no_inverse(seed, t, ego_iter, x_new, lambda_new, lambdas_all, weights_all):
    """
    ...
    for i in range(N_MC):
        record = {
            ...
            'timestep': t,
            'ego_iter': ego_iter,
            'EGO_lambda': lambda_new.item(),  # 改为直接记录 lambda 值
            'n_mc_idx': i,
        }
...
def update_data_switching_EGO_joint_gaussian(D_lambda_new, D_old, Y_old, n_rep, method, f, shape_x, known_sigma):

    D_new = D_lambda_new[:shape_x]
    lambda_new = D_lambda_new[shape_x:] 
    distribution_new = {'dist': "gaussian", "means": lambda_new, "sigmas": known_sigma}
