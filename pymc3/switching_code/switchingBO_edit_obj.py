...
def kde_sample(xi, size=1000): #newly added March27, 2025
    
    """
    使用 KDE 拟合正值数据，并确保采样结果均 > 0。

    参数:
    - xi: 输入数据 (numpy array, 要求所有值 > 0)
    - size: 生成的样本数量 (默认 1000)

    返回:
    - samples: 形状为 (size,) 的非负随机样本 (numpy array)
    """
    # 移除所有值为 0 的数据
    xi = xi[xi > 0]
...
#def output(file_pre, minimizer,minimum_value, f_hat_x, TIME_RE, x_star, f_star, seed, n_sample, iteration, n_xi, S):
    ...
    #print(x_star.shape,f_star.shape, 
          #minimizer.shape, f_hat_x.shape, xGAP.shape, GAP.shape )
...
class synthetic_uni_problem: # added Apr17, 2025
    ...
    def evaluate(self, xx, P, n_rep, method):
        """
        xx: 传入的决策变量值 x
        P: 分布参数(字典形式，包含 mu, sigma)
        n_rep: 仿真次数
        method: 使用的仿真方法(例如 'normal')
        """
        x = xx[0] 
        value = synthetic_simulator1(x, P, n_rep, method)  
...
def synthetic_simulator1(x, P, n_rep, method):
    """
    ...
    if method == 'bayesian_hist':  # KDE-based sampling from historical data
        xi_samples = kde_sample(P, size=n_rep)
        ...
        xi_samples = posterior_mean_and_generate_gaussian(xi=P, size=n_rep) 
...
def synthetic_objective1(x, xi):
    ##follow Enlu Zhou 2024 OR: Data-Driven Ranking and Selection Under IU, but add 2*xi to make the different optimal objective value
    return (x-xi)**2 + 2*xi 

def synthetic_true_objective1(x, theta_c, known_sigma):

    return (x-theta_c)**2 + known_sigma**2 + 2*theta_c
...
def bayesian_hmm_posterior_gaussian_known_var(n_draws, r_seed, obs, n_components, mu_prior_mean=5, mu_prior_std=5, known_std = 4):
    """
...
def sort_mcmc_posterior_by_mu(posterior, n_components): 
    ###added in Apr 17, 2025, remove label switching in bayesian hmm posterior sampling
    ###deal with parameter mu in gaussian distribution (increasing order)
    # 1. 提取 mu 和 p 的采样
    mu_samples = np.stack([
        posterior.posterior[f"mu_{i+1}"].values.reshape(-1)
        for i in range(n_components)
    ])  # shape: (n_components, n_samples)

    p_samples = np.stack([
        posterior.posterior[f"p_{i+1}"].values.reshape(-1, n_components)
        for i in range(n_components)
    ])  # shape: (n_components, n_samples, n_components)

    n_samples = mu_samples.shape[1]

    # 2. 创建存储排序后的结果
    sorted_mu_array = np.zeros_like(mu_samples)
    sorted_P_array = np.zeros_like(p_samples)

    # 3. 对每一个 sample 按 mu 值排序，并调整转移矩阵
    for i in range(n_samples):
        idx = np.argsort(mu_samples[:, i])  # 找到从小到大的索引
        sorted_mu_array[:, i] = mu_samples[idx, i]

        # 转移矩阵排序
        P_sample = p_samples[:, i, :]          # shape (n_components, n_components)
        P_sorted = P_sample[idx, :]            # row perm
        P_sorted = P_sorted[:, idx]            # col perm
        sorted_P_array[:, i, :] = P_sorted

    # 4. 返回格式和你原来一样
    lam_list = [sorted_mu_array[i:i+1, :] for i in range(n_components)]
    p_list = [sorted_P_array[i:i+1, :, :] for i in range(n_components)]
    return lam_list, p_list
