...
def EGO_posterior_mean_plug_in(seed):
    ...
    plug_lam_mean = bayesian_posterior_mean_uniform_prior(P, mu_prior_lower, mu_prior_upper, known_sigma, mean_sample_count, seed)

    D_update = np.concatenate((D1, np.full((D1.shape[0], 1), plug_lam_mean)), axis=1) ##joint modeling
    X_update = np.unique(D_update[:,:dimension_x], axis = 0)

    Y_list = list()
    for i in range(D1.shape[0]):
        xx = D1[i,:dimension_x] 
        P1 = {'dist': "gaussian", "sigmas": known_sigma, "means": plug_lam_mean}
    ...
    mu_g, sigma_g = predicted_mean_std_single_lambda_direct(X_, plug_lam_mean, gp0)
    mu_evaluated, _ = predicted_mean_std_single_lambda_direct(X_update, plug_lam_mean, gp0)
    ...
    for t in range(n_timestep):
        ...
        df.to_csv('log_gaussian_syn2_3regime_50data_no_RS_plug.csv', mode='a', header=not os.path.exists('log_gaussian_syn2_3regime_50data_no_RS_plug.csv'), index=False)
        ...
        for i in range(n_iteration): 
            ...
            distribution1_updated = {'dist': "gaussian", "sigmas": known_sigma, "means": plug_lam_mean}
...
cmd = ['-t','20260603-no-RS-plug-gaus-syn2-3regime-50data-seed-60-69', 
