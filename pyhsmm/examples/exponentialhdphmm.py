...
##self-develop the codes for exponential emission distribution in HDPHMM
class Exponential(GibbsSampling, Collapsed, MaxLikelihood, MeanField, MeanFieldSVI):
    '''
    Exponential distribution with a conjugate Gamma prior.

    NOTE: the support is {x >= 0}

    Hyperparameters (following Wikipedia's notation):
        alpha_0, beta_0

    Parameter is the rate parameter:
        lmbda 
    '''
    ...
    # 修改：Exponential 分布的 log_sf
    #def log_sf(self, x):
        #return stats.expon.logsf(x, scale=1/self.lmbda)  # Exponential 分布的 log survival function
    
    def log_sf(self, x):
        if self.lmbda is None or self.lmbda <= 0:
            raise ValueError("Lambda must be positive and initialized before calling log_sf.")
        return stats.expon.logsf(x, scale=1/self.lmbda)

    def _posterior_hypparams(self, n, tot):
        return self.alpha_0 + n, self.beta_0 + tot

    # 修改：Exponential 分布的采样
    #def rvs(self, size=None):
        #return np.random.exponential(1/self.lmbda, size=size)  # Exponential 分布的采样
    
    def rvs(self, size=None):
        if self.lmbda is None or self.lmbda <= 0:
            raise ValueError("Lambda must be positive before sampling.")
        return np.random.exponential(1/self.lmbda, size=size)


    # 修改：Exponential 分布的 log-likelihood
    def log_likelihood(self, x):
        lmbda = self.lmbda
        x = np.array(x, ndmin=1)
        raw = np.empty(x.shape)
        raw[x >= 0] = np.log(lmbda) - lmbda * x[x >= 0]  # Exponential 分布的对数似然
        raw[x < 0] = -np.inf
    ...
    #def resample(self, data=[], stats=None):
        ...
        #self.lmbda = np.random.gamma(alpha_n, 1/beta_n)  # 使用 Gamma 分布更新 lambda
    ...
    def resample(self, data=[], stats=None):
        ...
        # 这里修正 mean-field 初始化
        self.mf_alpha_0, self.mf_beta_0 = alpha_n, beta_n  
    ...
    def _resample_from_mf(self):
        ...
        self.lmbda = np.random.gamma(mf_alpha_0, 1./mf_beta_0)  # 使用 Gamma 分布更新 lambda
    ...
    # 修改：Exponential 分布的期望对数似然
    #def expected_log_likelihood(self, x):
        #Emlmbda, Elnlmbda = self._mf_expected_statistics
        #return Elnlmbda * x - Emlmbda * x  # Exponential 分布的期望对数似然
    
    def expected_log_likelihood(self, x):
        Emlmbda, Elnlmbda = self._mf_expected_statistics
        return Elnlmbda - Emlmbda * x  # 修正公式
    ...
    def max_likelihood(self, data, weights=None):
        ...
        if n > 1e-2:
            self.lmbda = n / tot  # 使用最大似然估计更新 lambda
            assert self.lmbda > 0
        else:
            self.broken = True
            self.lmbda = 999999
